...
class BrightnessApp extends BaseApp {
  ...
  _loadSettings() {
    this.brightness = parseFloat(localStorage.getItem("yukiOS_brightness")) || 100;
    this.contrast = parseFloat(localStorage.getItem("yukiOS_contrast")) || 1;
    this.gamma = parseFloat(localStorage.getItem("yukiOS_gamma")) || 1;
    this.temperature = parseFloat(localStorage.getItem("yukiOS_temperature")) || 50;
    this.nightModeEnabled = localStorage.getItem("yukiOS_night_mode_enabled") === "true";
    this.nightModeStart = localStorage.getItem("yukiOS_night_mode_start") || "20:00";
    this.nightModeEnd = localStorage.getItem("yukiOS_night_mode_end") || "07:00";
    this._applyDisplaySettings();
  }

  _saveSettings() {
    localStorage.setItem("yukiOS_brightness", this.brightness.toString());
    localStorage.setItem("yukiOS_contrast", this.contrast.toString());
    localStorage.setItem("yukiOS_gamma", this.gamma.toString());
    localStorage.setItem("yukiOS_temperature", this.temperature.toString());
    localStorage.setItem("yukiOS_night_mode_enabled", this.nightModeEnabled.toString());
    localStorage.setItem("yukiOS_night_mode_start", this.nightModeStart);
    localStorage.setItem("yukiOS_night_mode_end", this.nightModeEnd);
  }
  ...
  _adjustBrightness(delta) {
    ...
    if (!this._shouldSuppressNotification()) {
      this.notify("Brightness", `${Math.round(this.brightness)}%`, "info", 1000, "fa-sun");
    }
  }
  ...
  _adjustTemperature(delta) {
    ...
    this._updatePopupSliders();
    if (!this._shouldSuppressNotification()) {
      this.notify("Temperature", this._getTemperatureLabel(this.temperature), "info", 1000, "fa-temperature-half");
    }
  }
  ...
  _bindEvents(popup) {
    ...
    if (nightModeToggle) {
      nightModeToggle.addEventListener("change", (e) => {
        ...
        if (this.nightModeEnabled) {
          this._checkNightMode();
          if (!this._shouldSuppressNotification()) {
            this.notify("Night Mode", "Scheduled night mode enabled", "info", 2000, "fa-moon");
          }
        } else {
          if (!this._shouldSuppressNotification()) {
            this.notify("Night Mode", "Scheduled night mode disabled", "info", 2000, "fa-sun");
          }
        }
      });
    }
    ...
    if (resetBtn) {
      resetBtn.addEventListener("click", () => {
        ...
        if (!this._shouldSuppressNotification()) {
          this.notify("Reset", "Display settings reset to default", "info", 2000, "fa-undo");
        }
      });
    }
  }
