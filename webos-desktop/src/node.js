...
import { appendOutput, saveAs, handleTabKey, handleCtrlEnterRun } from "./shared/editorUtils.js";
...
export class NodeEditorApp {
  ...
  open(title = "NodeJS Code Editor", content = "", filePath = null) {
    ...
    this.wm.addToTaskbar(win.id, `${title} — NodeJS Code Editor`, "fab fa-js-square", "#caca43");
  ...
  _bindControls(win, currentFilePath) {
    ...
    codeArea.addEventListener("keydown", (e) => {
      handleTabKey(e, codeArea);
      handleCtrlEnterRun(e, runBtn);
    });
  ...
  _appendOutput(outputArea, text, type = "stdout") {
    appendOutput(outputArea, text, type);
  }

  _saveAs(fileName, content) {
    saveAs(this.fs, this.wm, fileName, content);
  }
}
