...
export class SettingsApp extends BaseApp {
  ...
  _renderDataSettings() {
    return `
      <div id="pane-data" class="settings-category-pane">
        ...
        <div class="settings-card" style="margin-top: 16px;">
          <div class="settings-card-header"><i class="fas fa-database"></i> Data Editor</div>
          <div class="settings-row">
            <div class="settings-label-group">
              <span class="settings-label-title">Storage Inspector</span>
              <span class="settings-label-desc">Browse and edit localStorage and IndexedDB keys</span>
            </div>
            <button class="settings-btn" id="btnOpenDataEditor">
              <i class="fas fa-table-cells"></i> Open
            </button>
          </div>
        </div>
  ...
  _bindDataCategory(win, showStatus, save) {
    ...
    win.querySelector("#btnOpenDataEditor")?.addEventListener("click", () => this._openDataEditor());
  ...
  _openDataEditor() {
    ...
    const win = this.wm.createWindow(winId, "Storage Editor", "900px", "600px");
    win.innerHTML = `
      <div class="window-header">
        <span>Storage Editor</span>
        ${this.wm.getWindowControls()}
      </div>
      <div class="window-content" style="padding:0;overflow:hidden;display:flex;flex-direction:column;height:calc(100% - 40px);">
        <div style="display:flex;gap:0;border-bottom:1px solid rgba(255,255,255,0.08);background:rgba(0,0,0,0.18);flex-shrink:0;">
          <button id="de-tab-ls" class="de-tab de-tab-active" style="padding:10px 22px;background:none;border:none;border-bottom:2px solid #4fa3ff;color:#4fa3ff;font-size:0.9em;font-weight:600;cursor:pointer;transition:all 0.15s;"><i class="fas fa-hdd" style="margin-right:6px;"></i>localStorage</button>
          <button id="de-tab-idb" class="de-tab" style="padding:10px 22px;background:none;border:none;border-bottom:2px solid transparent;color:rgba(255,255,255,0.45);font-size:0.9em;font-weight:600;cursor:pointer;transition:all 0.15s;"><i class="fas fa-server" style="margin-right:6px;"></i>IndexedDB</button>
        </div>
        <div style="display:flex;flex:1;overflow:hidden;">
          <div id="de-list-panel" style="width:260px;min-width:200px;border-right:1px solid rgba(255,255,255,0.07);display:flex;flex-direction:column;overflow:hidden;flex-shrink:0;">
            <div style="padding:10px;border-bottom:1px solid rgba(255,255,255,0.07);flex-shrink:0;">
              <input id="de-search" placeholder="Search keys..." style="width:100%;box-sizing:border-box;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.1);border-radius:6px;padding:6px 10px;color:#fff;font-size:0.83em;outline:none;"/>
            </div>
            <div id="de-key-list" style="flex:1;overflow-y:auto;padding:6px;"></div>
            <div style="padding:8px 10px;border-top:1px solid rgba(255,255,255,0.07);flex-shrink:0;">
              <button id="de-add-key" style="width:100%;background:rgba(79,163,255,0.12);border:1px solid rgba(79,163,255,0.25);border-radius:6px;color:#4fa3ff;padding:6px;font-size:0.82em;cursor:pointer;transition:all 0.15s;" onmouseover="this.style.background='rgba(79,163,255,0.22)'" onmouseout="this.style.background='rgba(79,163,255,0.12)'"><i class="fas fa-plus"></i> New Key</button>
            </div>
          </div>
          <div id="de-edit-panel" style="flex:1;display:flex;flex-direction:column;overflow:hidden;padding:16px;gap:12px;">
            <div id="de-empty-state" style="flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;color:rgba(255,255,255,0.25);">
              <i class="fas fa-table-cells" style="font-size:2.5em;"></i>
              <span style="font-size:0.9em;">Select a key to inspect and edit</span>
            </div>
            <div id="de-editor-area" style="display:none;flex-direction:column;gap:10px;flex:1;">
              <div style="display:flex;align-items:center;gap:10px;flex-shrink:0;">
                <input id="de-key-input" placeholder="Key name" style="flex:1;background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:6px;padding:7px 10px;color:#fff;font-size:0.85em;outline:none;font-family:monospace;"/>
                <span id="de-key-type" style="font-size:0.75em;padding:3px 8px;border-radius:4px;background:rgba(255,255,255,0.07);color:rgba(255,255,255,0.45);white-space:nowrap;"></span>
              </div>
              <textarea id="de-val-input" spellcheck="false" style="flex:1;min-height:200px;resize:vertical;background:rgba(0,0,0,0.25);border:1px solid rgba(255,255,255,0.1);border-radius:6px;padding:10px;color:#c9d1d9;font-family:monospace;font-size:0.82em;outline:none;line-height:1.5;"></textarea>
              <div style="display:flex;gap:8px;flex-shrink:0;">
                <button id="de-save-btn" style="background:rgba(79,163,255,0.15);border:1px solid rgba(79,163,255,0.3);border-radius:6px;color:#4fa3ff;padding:7px 18px;font-size:0.85em;cursor:pointer;transition:all 0.15s;" onmouseover="this.style.background='rgba(79,163,255,0.28)'" onmouseout="this.style.background='rgba(79,163,255,0.15)'"><i class="fas fa-save"></i> Save</button>
                <button id="de-delete-btn" style="background:rgba(255,77,79,0.1);border:1px solid rgba(255,77,79,0.25);border-radius:6px;color:#ff4d4f;padding:7px 18px;font-size:0.85em;cursor:pointer;transition:all 0.15s;" onmouseover="this.style.background='rgba(255,77,79,0.22)'" onmouseout="this.style.background='rgba(255,77,79,0.1)'"><i class="fas fa-trash"></i> Delete</button>
                <span id="de-status" style="font-size:0.78em;color:rgba(255,255,255,0.35);align-self:center;margin-left:auto;transition:opacity 0.3s;"></span>
              </div>
            </div>
          </div>
        </div>
    ...
    const showEditorStatus = (msg, color = "rgba(255,255,255,0.45)") => {
    ...
    const selectKey = (keyEl, keyName, value, typeLabel, idbCtx = null) => {
      ...
      activeKeyEl = keyEl;
      if (keyEl) keyEl.style.background = "rgba(79,163,255,0.13)";
    ...
    const buildKeyItem = (label, onclick) => {
      const el = document.createElement("div");
      el.style.cssText =
        "padding:7px 10px;border-radius:5px;cursor:pointer;font-size:0.8em;font-family:monospace;color:rgba(255,255,255,0.7);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;transition:background 0.1s;";
      el.title = label;
      el.textContent = label;
      el.addEventListener("mouseover", () => {
        if (el !== activeKeyEl) el.style.background = "rgba(255,255,255,0.06)";
      });
      el.addEventListener("mouseout", () => {
        if (el !== activeKeyEl) el.style.background = "";
      });
    ...
    const loadLocalStorage = () => {
      ...
      keys.sort().forEach((key) => {
        ...
        const el = buildKeyItem(key, () => {
          selectKey(el, key, localStorage.getItem(key), "localStorage");
        });
      ...
      try {
        ...
        for (const dbInfo of dbs) {
          ...
          header.style.cssText =
            "font-size:0.72em;font-weight:600;color:rgba(255,255,255,0.35);padding:8px 10px 4px;text-transform:uppercase;letter-spacing:0.06em;";
            ...
            for (const storeName of storeNames) {
              ...
              allKeys.forEach((key) => {
                ...
                if (
                  ...
                  return;
                const el = buildKeyItem(`${storeName} › ${keyStr}`, async () => {
                  const tx2 = db.transaction(storeName, "readonly");
                  const s2 = tx2.objectStore(storeName);
        ...
        keyList.innerHTML = `<div style="padding:10px;color:rgba(255,77,79,0.7);font-size:0.8em;text-align:center;">Failed to enumerate databases</div>`;
      }
      if (!keyList.querySelector("div[style*='monospace']")) {
        const noKeys = document.createElement("div");
        noKeys.style.cssText = "padding:10px;color:rgba(255,255,255,0.25);font-size:0.8em;text-align:center;";
    ...
    const setActiveTab = (tab) => {
      currentTab = tab;
      const isLs = tab === "ls";
      tabLs.style.color = isLs ? "#4fa3ff" : "rgba(255,255,255,0.45)";
      tabLs.style.borderBottom = isLs ? "2px solid #4fa3ff" : "2px solid transparent";
      tabIdb.style.color = !isLs ? "#4fa3ff" : "rgba(255,255,255,0.45)";
      tabIdb.style.borderBottom = !isLs ? "2px solid #4fa3ff" : "2px solid transparent";
      emptyState.style.display = "flex";
      editorArea.style.display = "none";
      activeKeyEl = null;
      if (isLs) loadLocalStorage();
      else loadIdb();
    };

    tabLs.addEventListener("click", () => setActiveTab("ls"));
    tabIdb.addEventListener("click", () => setActiveTab("idb"));
    searchInput.addEventListener("input", () => (currentTab === "ls" ? loadLocalStorage() : loadIdb()));
    ...
    saveBtn.addEventListener("click", async () => {
      ...
      if (currentTab === "ls" || !currentIdbCtx) {
    ...
    addKeyBtn.addEventListener("click", () => {
      ...
      keyInput.value = "";
      valInput.value = "";
      keyType.textContent = currentTab === "ls" ? "localStorage" : "IDB";
      keyInput.focus();
    });
