...
export class InstalledAppsApp extends BaseApp {
  ...
  open(options = {}) {
    ...
    const win = this.wm.createWindow(winId, "Installed Apps", "900px", "650px", false, options);

    win.innerHTML = this._buildHTML();

    this.windowHelper.mountWindow(win, winId, "Installed Apps", "fas fa-th-list");
  ...
  _getAppIcon(app) {
    if (app.icon) {
      if (app.icon.startsWith("fas ") || app.icon.startsWith("far ") || app.icon.startsWith("fab ")) {
