...
class SceneObject {
  constructor(threeObj, name, type = "mesh") {
    ...
    this.type = type; // mesh | group | light | camera | bone
    this.visible = true;
    this.selected = false;
    this.locked = false;
    this.children = [];
...
export class Model3DApp extends BaseApp {
  constructor(services) {
    ...
    // Renderer state
    this.renderMode = "lit"; // lit | wireframe | solid | texture
    ...
    // Post-processing toggles (stored as flags; real PP requires EffectComposer)
    ...
    // Three objects (set in setupRenderer)
    this.scene = null;
    this.camera = null;
    this.renderer = null;
    this.controls = null;
  ...
  async open(titleOrOptions = "Yuki Blender", filePath = null) {
    ...
    const win = this.wm.createWindow(winId, title, "1080px", "640px", false, {
  ...
  _setupMenus() {
    const win = this.win;
    const menuItems = win.querySelectorAll(".yb-menu-item");
    let activeMenu = null;
    const closeAll = () => {
    ...
    // Action items
    win.querySelectorAll(".yb-dropdown-item[data-action]").forEach((item) => {
      item.addEventListener("click", (e) => {
        e.stopPropagation();
        const action = item.dataset.action;
        const isToggle = item.classList.contains("toggle-item");
        if (isToggle) {
          const chk = item.querySelector(".yb-check");
    ...
    // Render mode items
    win.querySelectorAll(".rm-item[data-rm]").forEach((item) => {
      item.addEventListener("click", (e) => {
        e.stopPropagation();
        win.querySelectorAll(".rm-item .yb-check").forEach((c) => (c.style.visibility = "hidden"));
        item.querySelector(".yb-check").style.visibility = "visible";
    ...
    // Background items
    win.querySelectorAll(".bg-item[data-bg]").forEach((item) => {
      item.addEventListener("click", (e) => {
        e.stopPropagation();
        win.querySelectorAll(".bg-item .yb-check").forEach((c) => (c.style.visibility = "hidden"));
        item.querySelector(".yb-check").style.visibility = "visible";
  ...
  _setupToolbar() {
    ...
    win.querySelectorAll(".yb-tool-btn[data-tool]").forEach((btn) => {
    ...
    win.querySelectorAll(".yb-tool-btn[data-rm]").forEach((btn) => {
      btn.addEventListener("click", () => this._setRenderMode(btn.dataset.rm));
    });

    win.querySelectorAll(".yb-tool-btn[data-action]").forEach((btn) => {
  ...
  _setActiveTool(tool) {
    ...
    this.win.querySelectorAll(".yb-tool-btn[data-tool]").forEach((b) => {
      b.classList.toggle("active", b.dataset.tool === tool);
    });
    const badge = this.win.querySelector("#yb-tool-badge");
    if (badge) badge.textContent = tool.toUpperCase();
  ...
  _toggleTransformSpace() {
    ...
    const lbl = this.win.querySelector("#tb-space");
    if (lbl) lbl.textContent = this.transformSpace.toUpperCase();
    if (this.transformControls) {
  ...
  // ── Inspector tabs ────────────────────────────────────────────────────────
  _setupInspectorTabs() {
    this.win.querySelectorAll(".yb-ins-tab").forEach((tab) => {
      tab.addEventListener("click", () => {
        this.win.querySelectorAll(".yb-ins-tab").forEach((t) => t.classList.remove("active"));
        this.win.querySelectorAll(".yb-ins-panel").forEach((p) => p.classList.remove("active"));
        tab.classList.add("active");
        const panel = this.win.querySelector(`#yb-ins-${tab.dataset.tab}`);
  ...
  _setupInspectorControls() {
    ...
    // Light sliders
    win.querySelectorAll(".yb-slider[data-light]").forEach((slider) => {
      slider.oninput = () => {
    ...
    // Exposure
    const expSlider = win.querySelector("#exposure-slider");
    if (expSlider) {
      expSlider.oninput = () => {
        this.exposure = parseFloat(expSlider.value);
        if (this.renderer) this.renderer.toneMappingExposure = this.exposure;
        const lbl = win.querySelector("#exposure-val");
        if (lbl) lbl.textContent = this.exposure.toFixed(1);
      };
    }

    // Grid controls
    const gridSizeSlider = win.querySelector("#grid-size");
    const gridDivsSlider = win.querySelector("#grid-divs");
    if (gridSizeSlider) {
      gridSizeSlider.oninput = () => {
        const lbl = win.querySelector("#grid-size-val");
        if (lbl) lbl.textContent = gridSizeSlider.value;
        this._rebuildGrid(parseInt(gridSizeSlider.value), parseInt(gridDivsSlider?.value || 20));
      };
    }
    if (gridDivsSlider) {
      gridDivsSlider.oninput = () => {
        const lbl = win.querySelector("#grid-divs-val");
        if (lbl) lbl.textContent = gridDivsSlider.value;
        this._rebuildGrid(parseInt(gridSizeSlider?.value || 20), parseInt(gridDivsSlider.value));
      };
    }

    // Snap
    const snapToggle = win.querySelector("#snap-toggle");
    if (snapToggle) {
      snapToggle.onchange = () => {
        this.snapEnabled = snapToggle.checked;
        const lbl = win.querySelector("#tb-snap");
    ...
    const snapStepSlider = win.querySelector("#snap-step");
    if (snapStepSlider) {
      snapStepSlider.oninput = () => {
        this.snapGrid = parseFloat(snapStepSlider.value);
        const lbl = win.querySelector("#snap-step-val");
        if (lbl) lbl.textContent = this.snapGrid.toFixed(2);
    ...
    ["tx-x", "tx-y", "tx-z", "rx-x", "rx-y", "rx-z", "sx-x", "sx-y", "sx-z"].forEach((id) => {
      const inp = win.querySelector(`#${id}`);
      if (inp) inp.onchange = () => this._applyTransformFromInspector();
    });

    // Visibility checkboxes
    const objVisible = win.querySelector("#obj-visible");
    if (objVisible)
      objVisible.onchange = () => {
    ...
    const objCast = win.querySelector("#obj-cast-shadow");
    if (objCast)
      objCast.onchange = () => {
        const sel = this._getFirstSelected();
        if (sel)
    ...
    const objRecv = win.querySelector("#obj-recv-shadow");
    if (objRecv)
      objRecv.onchange = () => {
        const sel = this._getFirstSelected();
        if (sel)
    ...
    // Texture assign
    win.querySelectorAll(".yb-tex-btn[data-tex]").forEach((btn) => {
      btn.onclick = () => this._assignTexture(btn.dataset.tex);
    });

    // Outliner add button
    const addBtn = win.querySelector(".yb-panel-btn[data-action='addCube']");
  ...
  _setupMaterialControls() {
    ...
    const syncMat = () => {
      ...
      const col = win.querySelector("#mat-color");
      if (col) mat.color?.setStyle(col.value);
      const rough = win.querySelector("#mat-roughness");
      if (rough && mat.roughness !== undefined) mat.roughness = parseFloat(rough.value);
      const metal = win.querySelector("#mat-metalness");
      if (metal && mat.metalness !== undefined) mat.metalness = parseFloat(metal.value);
      const opac = win.querySelector("#mat-opacity");
      if (opac) {
        mat.opacity = parseFloat(opac.value);
        mat.transparent = mat.opacity < 1;
      }
      const wf = win.querySelector("#mat-wireframe");
      if (wf) mat.wireframe = wf.checked;
      const ds = win.querySelector("#mat-double-sided");
      ...
      (id) => {
        ...
        if (el) {
          el.addEventListener("input", () => {
            const valEl = win.querySelector(`#${id}-val`);
            if (valEl) valEl.textContent = parseFloat(el.value).toFixed(2);
            syncMat();
          });
          el.addEventListener("change", syncMat);
        }
      }
    );
  }
  ...
  // ── Drag & drop ───────────────────────────────────────────────────────────
  _setupDragDrop() {
    const viewport = this.win.querySelector("#yb-viewport");
    const overlay = this.win.querySelector("#yb-drag-overlay");
    viewport.addEventListener("dragover", (e) => {
      e.preventDefault();
      overlay.classList.add("active");
    });
    viewport.addEventListener("dragleave", () => overlay.classList.remove("active"));
    viewport.addEventListener("drop", async (e) => {
      ...
      this.wm.sendNotify(`Loaded: ${file.name}`, "Load");
    });
  }
  ...
  async _setupRenderer(filePath, fileData, fileName) {
    ...
    const host = this.win.querySelector("#yb-canvas-host");
  ...
  _setRenderMode(mode) {
    ...
    // sync toolbar rm buttons
    this.win.querySelectorAll(".yb-tool-btn[data-rm]").forEach((b) => {
  ...
  _refreshOutliner() {
    const outliner = this.win.querySelector("#yb-outliner");
    if (!outliner) return;
    if (this.sceneObjects.size === 0) {
      outliner.innerHTML = '<div class="yb-outliner-empty">No objects in scene</div>';
      return;
    }
    outliner.innerHTML = "";
  ...
  _syncTransformToInspector() {
    ...
    const set = (id, v) => {
      const el = this.win.querySelector(`#${id}`);
    ...
    set("sx-y", o.scale.y);
    set("sx-z", o.scale.z);
    const insEmpty = this.win.querySelector("#ins-obj-empty");
    if (insEmpty) insEmpty.style.display = "none";
  }

  _syncMaterialToInspector() {
    const sel = this._getFirstSelected();
    const empty = this.win.querySelector("#ins-mat-empty");
    if (!sel) {
    ...
    const slotEl = this.win.querySelector("#ins-mat-slots");
    if (slotEl) {
    ...
    if (!mat) return;
    const get = (id) => this.win.querySelector(`#${id}`);
    const setVal = (id, v) => {
      const el = get(id);
      if (el) el.value = v;
    };
  ...
  _applyTransformFromInspector() {
    ...
    const get = (id) => parseFloat(this.win.querySelector(`#${id}`)?.value || 0);
  ...
  _updateSelectionInfo() {
    const info = this.win.querySelector("#yb-sel-info");
    const lbl = this.win.querySelector("#yb-sel-label");
    if (!info || !lbl) return;
    ...
    const sel = this._getFirstSelected();
    lbl.textContent = sel ? sel.name : `${this.selectedIds.size} objects`;
  ...
  _addPrimitive(type) {
    ...
    const hud = this.win.querySelector("#yb-hud");
    if (hud) hud.style.display = "block";
    this.wm.sendNotify(`Added ${name}`, "Scene");
  }
  ...
  _showLoading(show, text = "Loading…") {
    const el = this.win.querySelector("#yb-loading");
    const tx = this.win.querySelector("#yb-loading-text");
    if (el) el.style.display = show ? "flex" : "none";
    if (tx) tx.textContent = text;
  }

  _hideWelcome() {
    const w = this.win.querySelector("#yb-welcome");
  ...
  _newScene() {
    ...
    const tl = this.win.querySelector("#yb-timeline");
    if (tl) tl.style.display = "none";
    const welcome = this.win.querySelector("#yb-welcome");
    if (welcome) welcome.style.display = "flex";
    const sbFile = this.win.querySelector("#sb-file");
    if (sbFile) sbFile.innerHTML = '<i class="fa fa-file"></i> No model';
    this.wm.sendNotify("New scene created", "Scene");
  }
  ...
  async loadModel(fileData, fileName = "") {
    ...
    // Remove previous root model
    if (this.currentModel) {
      // Remove from sceneObjects map if it's in there
    ...
    try {
      ...
      this.wm.sendNotify(`Failed to load ${fileName}: ${err.message}`, "Error");
    }
    ...
    try {
      ...
      for (const [fname, bytes] of Object.entries(unzipped)) {
        ...
        if (supported.includes(ext)) {
          ...
          this.wm.sendNotify(`Loaded ${fname} from zip`, "Load");
          return true;
        }
      }
      this.wm.sendNotify("No supported models found in zip", "Error");
      return false;
    } catch (err) {
      this.wm.sendNotify("Failed to read zip file", "Error");
      return false;
    }
  }
  ...
  _updateFileStatus(fileName) {
    const el = this.win.querySelector("#sb-file");
    if (el) el.innerHTML = `<i class="fa fa-file"></i> ${fileName}`;
  }
  ...
  _setupSkeleton(object) {
    ...
    if (skinnedMesh && skeleton) {
      ...
      ["#ins-bone-tree", "#ins-bone-tree2"].forEach((id) => {
        const el = this.win.querySelector(id);
        if (el) el.innerHTML = treeHtml || "<span class='yb-empty-label'>–</span>";
      });
  ...
  _setupAnimationTimeline() {
    ...
    win.querySelectorAll(".yb-tl-btn[data-anim]").forEach((btn) => {
      btn.onclick = () => this._handleAnimTransport(btn.dataset.anim);
    });

    const scrubber = win.querySelector("#tl-scrubber");
    if (scrubber) {
      scrubber.addEventListener("mousedown", () => (this.isDraggingTimeline = true));
      scrubber.addEventListener("mouseup", () => (this.isDraggingTimeline = false));
    ...
    const speed = win.querySelector("#tl-speed");
    if (speed) {
      speed.onchange = () => {
    ...
    const loop = win.querySelector("#tl-loop");
    if (loop) {
      loop.onchange = () => {
    ...
    const tlClose = win.querySelector("#yb-tl-close");
  ...
  _refreshAnimations(animations) {
    const list = this.win.querySelector("#yb-anim-list");
    const empty = this.win.querySelector("#ins-anim-empty");
    if (!list) return;

    if (animations.length === 0) {
      list.innerHTML = '<span class="yb-empty-label">No animations</span>';
      if (empty) empty.style.display = "flex";
      const sb = this.win.querySelector("#sb-anim");
      if (sb) sb.innerHTML = '<i class="fa fa-film"></i> –';
      return;
    }
    if (empty) empty.style.display = "none";
  ...
  _duplicateSelected() {
    ...
    this.wm.sendNotify(`Duplicated: ${sel.name}`, "Scene");
  }
  ...
  _deleteSelected() {
    ...
    this.wm.sendNotify(`Deleted ${ids.length} object(s)`, "Scene");
  }
  ...
  _toggleOrtho() {
    ...
    this.wm.sendNotify(this.isOrtho ? "Orthographic view" : "Perspective view", "View");
  }

  // ── Export ────────────────────────────────────────────────────────────────
  async _exportGLTF() {
    if (!this.currentModel) {
      this.wm.sendNotify("No model to export", "Error");
      return;
    }
    try {
      const baseUrl = getLibraryUrl("three");
      ...
      exporter.parse(
        ...
        (result) => {
          ...
          this.wm.sendNotify("Exported GLTF", "Export");
        },
        (err) => this.wm.sendNotify("Export failed: " + err, "Error"),
        { binary: false }
      );
    } catch (e) {
      this.wm.sendNotify("GLTF exporter not available", "Error");
    }
  }
  ...
  _exportOBJ() {
    ...
    this.wm.sendNotify("Exported OBJ", "Export");
  }
  ...
  _exportSceneJSON() {
    ...
    this.wm.sendNotify("Scene JSON exported", "Export");
  }
  ...
  _recalcNormals() {
    const sel = this._getFirstSelected();
    if (!sel) {
      this.wm.sendNotify("Select an object first", "Error");
      return;
    }
    sel.object3D.traverse((c) => {
      if (c.isMesh) c.geometry.computeVertexNormals();
    });
    this.wm.sendNotify("Normals recalculated", "Mesh");
  }
  ...
  _applyTransformsToMesh() {
    ...
    this.wm.sendNotify("Transforms applied", "Mesh");
  }
  ...
  _assignTexture(type) {
    ...
    input.onchange = async (e) => {
      ...
      const texture = new THREE.TextureLoader().load(url, () => {
        ...
        this._syncMaterialToInspector();
        this.wm.sendNotify(`${type} texture assigned`, "Material");
      });
    };
    input.click();
  }
  ...
  _openFromExplorer() {
    ...
    this.explorerApp.open(async (path, fileName) => {
      const ab = await this.fs.getFileContent(path, fileName, "arraybuffer");
      await this.loadModel(ab, fileName);
      this.wm.sendNotify(`Loaded: ${fileName}`, "Load");
    }, this);
  }
  ...
  async _takeScreenshot() {
    try {
      ...
      await this.fs.writeBinaryFile(["Pictures"], name, blob, "image", dataUrl);
      this.wm.sendNotify(`Screenshot saved: Pictures/${name}`, "Screenshot");
    } catch (err) {
      console.error(err);
      this.wm.sendNotify("Screenshot failed", "Error");
    }
  }
  ...
  async _loadSampleModel(index, fillEl, textEl, card) {
    ...
    this.wm.sendNotify(`Loaded: ${sample.name}`, "Load");
  }
}
